...
Architecture
------------
This module is the entrypoint that starts 6 concurrent systems:
...
from notifications import send_telegram_alert  # noqa: F401
...
async def precompute_heavy_analytics():
    ...
    try:
        ...
        mc_rows = await pg_fetch_all("""
            ...
            INNER JOIN (SELECT pos_id, MIN(id) as first_snap_id FROM position_snapshots GROUP BY pos_id) fs ON ps.id = fs.first_snap_id
            INNER JOIN open_positions op ON ps.pos_id = op.id
...
async def scoring_quality_api():
    ...
    try:
        ...
        approved_stats = await pg_fetch_one("""
            ...
            WHERE t.event_type IN ('full_close', 'sl_hit', 'tp3_hit', 'timeout', 'apollo_full_exit', 'zone_full_exit', 'e_pnl_full_exit')
...
async def _compute_analytics():
    ...
    by_recommendation = await pg_fetch_all("""
        SELECT s.re_recommendation as rec,
           ...
           AVG(CASE WHEN t.event_type IN ('full_close','sl_hit','tp3_hit','timeout','apollo_full_exit','zone_full_exit','e_pnl_full_exit') THEN t.pnl_pct END) as avg_pnl,
           SUM(CASE WHEN t.pnl_pct > 0 AND t.event_type IN ('full_close','sl_hit','tp3_hit','timeout','apollo_full_exit','zone_full_exit','e_pnl_full_exit') THEN 1 ELSE 0 END) as wins,
           SUM(CASE WHEN t.pnl_pct <= 0 AND t.event_type IN ('full_close','sl_hit','tp3_hit','timeout','apollo_full_exit','zone_full_exit','e_pnl_full_exit') THEN 1 ELSE 0 END) as losses,
           ROUND(COALESCE(SUM(CASE WHEN t.event_type IN ('full_close','sl_hit','tp3_hit','timeout','apollo_full_exit','zone_full_exit','e_pnl_full_exit') THEN t.pnl_pct END), 0), 2) as sum_pnl_pct,
           ROUND(COALESCE(SUM(
               CASE WHEN t.event_type IN ('full_close','sl_hit','tp3_hit','timeout','apollo_full_exit','zone_full_exit','e_pnl_full_exit')
    ...
    try:
        ...
        try:
            hr_rows = await pg_fetch_all("""
                ...
                FROM signals s JOIN trade_outcomes t ON s.signal_hash = t.signal_hash
                WHERE t.event_type IN ('full_close','sl_hit','tp3_hit','timeout','tp1_hit','apollo_full_exit','zone_full_exit','e_pnl_full_exit')
                  AND COALESCE(s.source, '') != 'backtest_v2'""")
...
async def score_outcome_api():
    ...
    points = await pg_fetch_all("""
        ...
        WHERE s.re_signal_score IS NOT NULL
          AND t.event_type IN ('full_close','sl_hit','tp3_hit','timeout','apollo_full_exit','zone_full_exit','e_pnl_full_exit')
...
async def tv_webhook(payload: WebhookPayload, x_webhook_secret: str = Header(default="")):
    ...
    try:
        ...
        combined_approved = (recommendation == "approve") and mc_result.approved
        ...
        if normalized_var > 3.0 and recommendation == "approve":
            ...
            combined_approved = False
            logger.info(f"VaR override: {var_pct*100:.2f}% (normalized={normalized_var:.1f}x baseline), downgrading to reduce")
        ...
        if _close_candidate_id and recommendation in _close_min_rec:
            ...
            try:
                ...
                if close_result.get("ok"):
                    await pg_execute(
                        "UPDATE open_positions SET status='closed', close_reason=?, closed_at=now() WHERE id=?",
                        (_close_reason_tag, _close_candidate_id)
                    )
                    logger.info(
...
async def health_check():
    ...
    try:
        ...
        stale_thresholds = {
            ...
            "exit_quality": 1200,           # 20 min (runs every 15 min) — v0.18.0
        }
        ...
        return {"status": "unhealthy", "error": str(e), "version": "0.18.3"}
