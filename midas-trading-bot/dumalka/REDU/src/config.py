...
Updated: 2026-04-08 (v0.19.6 — ML Shadow Mode + ATR-adaptive TP1 Soft BE skip)
...
class Config:
    ...
    # Hard SL Cap: max loss % per position before force-close
    # v0.18.8 (2026-04-04): ACTIVATED at 3.5%. Backtest: PF 0.946→1.263 on 100 approve-only trades.
    # With ~0.5% slippage budget, effective cap ~4.0%. Cycle is 30s.
    MAX_LOSS_PCT = float(os.getenv("MAX_LOSS_PCT", "3.5"))
    # SHADOW MODE: log Phase 1 decisions but do NOT intervene in trading
    # ACTIVE MODE: send full_close when loss exceeds MAX_LOSS_PCT (takes precedence over shadow)
    ...
    # Kline Historical Collector (v0.19.0)
    # Stores 15m/1h/4h candles in PostgreSQL for Scout signals and future ML.
    # 2-tier fetch: OKX (geo-free) → Bybit Proxy (100% coverage).
    KLINE_COLLECTOR_ENABLED = os.getenv("KLINE_COLLECTOR_ENABLED", "true").lower() == "true"
    KLINE_COLLECT_INTERVAL_SEC = int(os.getenv("KLINE_COLLECT_INTERVAL_SEC", "60"))

    # Scout Signal Generator (v0.19.0) — SHADOW MODE ONLY
    # Generates autonomous EMA/RSI/Regime signals, tracks shadow PnL.
    # Does NOT send to bot or trigger any trades.
    SCOUT_ENABLED = os.getenv("SCOUT_ENABLED", "true").lower() == "true"
    SCOUT_INTERVAL_SEC = int(os.getenv("SCOUT_INTERVAL_SEC", "900"))  # 15 min

    # v0.19.6 (2026-04-08): ML Shadow Mode — log ExtraTrees predictions without affecting trading.
    # Loads model from src/models/et_shadow_v1.pkl at startup. Writes to ml_predictions table.
    ML_SHADOW_ENABLED = os.getenv("ML_SHADOW_ENABLED", "true").lower() == "true"
    ML_SHADOW_CONFIDENCE_THRESHOLD = float(os.getenv("ML_SHADOW_CONFIDENCE_THRESHOLD", "0.65"))

    # v0.19.4 (2026-04-05): ATR-adaptive TP1 Soft BE
    # When TP1 distance < TP1_BE_ATR_SKIP_MULT × ATR(1h), skip BE move entirely.
    # Rationale: TP1 in the noise zone → BE SL gets stopped by normal oscillation.
    # Validated on 7/7 cases since 27.03 — skip equal or better in all.
    TP1_BE_ATR_SKIP_MULT = float(os.getenv("TP1_BE_ATR_SKIP_MULT", "2.0"))
    TP1_BE_OFFSET_MULT = float(os.getenv("TP1_BE_OFFSET_MULT", "0.5"))
    TP1_BE_MIN_PCT = float(os.getenv("TP1_BE_MIN_PCT", "0.005"))   # 0.5% floor
    TP1_BE_MAX_PCT = float(os.getenv("TP1_BE_MAX_PCT", "0.030"))   # 3.0% cap

    # v0.19.7 (2026-04-11): Health Watchdog — TG alerts on component failure / bot disconnect
    HEALTH_WATCHDOG_ENABLED = os.getenv("HEALTH_WATCHDOG_ENABLED", "true").lower() == "true"
    BOT_UNREACHABLE_ALERT_SEC = int(os.getenv("BOT_UNREACHABLE_ALERT_SEC", "420"))  # 7 min
    WATCHDOG_ALERT_COOLDOWN_SEC = int(os.getenv("WATCHDOG_ALERT_COOLDOWN_SEC", "1800"))  # 30 min re-alert
...
if not config.WEBHOOK_SECRET:
    _cfg_logger.critical(
        "WEBHOOK_SECRET is empty! /tv-webhook and /trade-outcome endpoints "
        "are UNPROTECTED — any request will be accepted without auth. "
        "Set WEBHOOK_SECRET env var immediately."
    )
