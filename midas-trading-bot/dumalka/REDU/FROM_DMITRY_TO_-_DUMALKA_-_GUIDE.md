...
|------|-----|-------------|----------|
| `action` | string | **Да** | `partial_close`, `move_sl`, `move_tp`, `full_close`, `place_limit_tp` (v0.14.2) |
| `symbol` | string | **Да** | Символ позиции (ETHUSDT) |
| `fraction` | float | Для partial_close / limit | Доля закрытия (0.0 — 1.0). По умолчанию 0.3 |
| `new_sl` | float | Для move_sl | Новая цена стоп-лосса (в v0.14.2 заменена на ATR-Adaptive) |
| `new_tp` | float | Для move_tp | Новая цена тейк-профита |
| `target_price` | float | Для place_limit_tp | Устанавливает Maker-ордер в стакане по этой цене |
