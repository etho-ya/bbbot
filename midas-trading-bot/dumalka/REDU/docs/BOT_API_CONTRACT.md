...
> **RE version**: 0.19.8+
> **Bot version**: 0.10.3+
> **Status**: ✅ Phase 2 complete — integration verified end-to-end (see `docs/INCIDENT_REPORT_2026_04_12.md`)
...
- [x] Implement `POST /tv-webhook` call with `source: "bot_direct"`
- [x] Read `approved` boolean from HTTP response as primary gate
- [x] Implement decision mapping: reject / reduce / approve
- [x] Use `conviction_size_usd` for position sizing
- [x] Handle `rejection_reason` for logging
- [x] Implement 25s timeout with 30% fallback
- [x] Set `auto_be_price` / `auto_be_trigger` on the exchange if provided
- [x] Stop posting "Request for approval" to Telegram channel
- [x] Stop listening for callback on `/api/re/callback` (now returns 410 Gone)

### Phase 2: Trade Events (replace Telegram → direct API)

- [x] Send `event: "open"` to `/trade-outcome` when position opens on exchange
- [x] Send `event: "tp1_hit"` / `"tp2_hit"` / `"tp3_hit"` on partial takes
- [x] Send `event: "sl_hit"` on stop-loss trigger
- [x] Send `event: "full_close"` / `"manual_close"` / `"flip_close"` as appropriate
- [x] Send `event: "dumalka_close"` when closing by RE command
- [x] Include `pnl_pct` and `price` in all close events
- [x] Include `hash` (signal_hash) in all events for signal chain linking
- [x] Stop posting trade event text to Telegram channel

### Phase 3: Verification

- [x] Confirm `/health` endpoint responds with `{"status":"healthy",...}`
- [x] Verify positions appear in RE dashboard after opening
- [x] Verify close events update RE dashboard within seconds (not minutes)
- [x] Confirm `linked_to_signal: true` in trade-outcome responses (fixed in round 3)
- [x] Monitor Telegram for formatted reports from RE (human observability)
- [x] Bot input validation: Literal action types + model_validator for params
- [x] Auth: `X-Webhook-Secret` verified both directions
- [x] Full end-to-end test: 100+ requests across all endpoints

### No changes needed

- [x] `/dumalka/command` — continues as-is (5 action types, Literal validation)
- [x] `/dumalka/positions` — continues as-is (polled every ~30s)
- [x] Dumalka fallback timeout (15 min without commands → classic mode) — continues as-is
