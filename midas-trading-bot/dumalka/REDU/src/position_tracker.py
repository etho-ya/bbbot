"""
Position Tracker — v0.19.6 "Думалка" Smart Execution

AI Position Management System with multi-level decision pipeline (every 30s):
  Level 0: Hard SL Cap — force-close at MAX_LOSS_PCT (ACTIVE since v0.18.8)
...
Key Subsystems:
  - v0.19.6: ML Shadow Mode — ExtraTrees+Optuna model (AUC 0.621) predicts profit
    probability after 5 position snapshots (~2.5 min). Logs to ml_predictions table.
    No trade impact — purely observational for out-of-sample validation. (2026-04-08)
  - v0.19.5: Enriched audit diagnostics — zone_full_exit, e_pnl_full_exit, and
    sl_breakeven now log volatility, adaptive_factor, vol_modifier, regime_dd,
    heat_modifier, zone name, and base threshold to mc_diagnostics JSON.
    Enables post-mortem analysis of triple compression (af*vol_mod*regime)
    without joining position_snapshots. (2026-04-07)
  - v0.19.4: ATR-adaptive TP1 Soft BE — skip BE move when TP1 is within ATR noise
    zone (tp1_dist < ATR(1h) × TP1_BE_ATR_SKIP_MULT). Old formula was capped at
    0.5% for all assets; new formula scales 0.5%-3.0% with ATR. Validated: 7/7
    cases since 27.03 — skip equal or better (+21.9% PnL recovered). (2026-04-05)
  - v0.18.9: sl_breakeven clamp fix — ATR offset could push SL past current price
    for SHORT positions (be_sl < market → Bybit rejected). Clamp ensures SL always
    between entry and current_price for both LONG and SHORT. (2026-04-04)
  - v0.18.8: Hard SL Cap ACTIVATED at 3.5% — force-close positions exceeding loss threshold.
    Backtest: PF 0.946→1.263 on 100 approve-only trades (2026-04-04)
  - v0.19.3: Telegram via BYBIT_PROXY /telegram/send first (geo-block bypass),
    fallback to api.telegram.org — same as notifications.send_telegram_message (2026-04-05)
  - v0.18.6: Structured Audit Log — pos_id, details JSONB, market_state enrichment,
    audit writes for position_closed, phantom_transition, observe_only,
    phantom_retry_ok, grace_proven, apollo_shadow, time_decay_exempt (2026-04-04)
  - v0.18.4: "Proven Position" — sticky grace bypass when max_pnl>=3% & tp>=50% (2026-04-02)
...
from db import insert_trade_outcome, insert_position_snapshot, update_snapshot_future_pnl, finalize_snapshot_future_pnl
...
# ── v0.18.4: Sticky "proven position" flag for grace period bypass ───────────
# Once a young (<1h) position reaches max_pnl >= 3% AND tp_progress >= 50%,
# it is marked as proven and zone policy / trailing are unblocked.
# Latching prevents flickering when tp_progress oscillates around the threshold.
# NOM #548 post-mortem: +14% -> -3.7% because grace blocked zone_full_exit.
_proven_positions: set[int] = set()

# v0.18.6: Track which positions already had observe_only audit written
# to avoid writing every cycle (~1/min). Written once on first detection.
_observe_only_audited: set[int] = set()

# v0.18.6: Shared market context for audit log enrichment.
# Updated once per cycle in track_open_positions; read by _write_audit_log.
_market_context: dict = {}

# v0.19.6 (2026-04-08): ML Shadow Mode — set by main.py at startup from joblib bundle.
# None = model not loaded; predictions skipped silently.
_ml_shadow_model = None
_ml_shadow_imputer = None
_ml_shadow_features: list = []
_ml_shadow_numeric: list = []
_ml_shadow_version: str = "unknown"
_ml_shadow_logged: set[int] = set()  # pos_ids already predicted this lifecycle


async def _ensure_db_columns():
    """Ensure required DB columns exist (idempotent migrations).

    v0.18.6: pos_id + details columns on dumalka_audit_log for structured
    audit of closes, phantom transitions, and state changes.
    """
    try:
        try:
        ...
        try:
            await pg_execute("ALTER TABLE dumalka_audit_log ADD COLUMN IF NOT EXISTS pos_id INTEGER")
            await pg_execute("ALTER TABLE dumalka_audit_log ADD COLUMN IF NOT EXISTS details JSONB")
            await pg_execute(
                "CREATE INDEX IF NOT EXISTS idx_audit_pos_id "
                "ON dumalka_audit_log (pos_id) WHERE pos_id IS NOT NULL"
            )
        except Exception as e:
            logger.debug(f"audit_log column migration skipped: {e}")
    except Exception as e:
        logger.error(f"Failed to ensure DB columns: {e}")


async def _ml_shadow_predict(pos_id: int, symbol: str, side: str,
                             snapshot_count: int):
    """Log ML shadow prediction for a position after enough snapshots collected.

    Uses ExtraTrees+Optuna model (v0.19.6, 2026-04-08) to predict profit
    probability from aggregated early-snapshot features. Writes to ml_predictions
    table. Does NOT affect any trading decision — purely observational.

    Called once per position lifecycle (after snapshot_count >= 5 and not yet
    predicted). Features are loaded from the latest position_snapshots rows.
    """
    global _ml_shadow_model, _ml_shadow_imputer, _ml_shadow_logged
    if _ml_shadow_model is None or not config.ML_SHADOW_ENABLED:
        return
    if pos_id in _ml_shadow_logged:
        return

    try:
        import numpy as np

        existing = await pg_fetch_val(
            "SELECT COUNT(*) FROM ml_predictions WHERE pos_id = ?", (pos_id,)
        )
        if existing and int(existing) > 0:
            _ml_shadow_logged.add(pos_id)
            return

        rows = await pg_fetch_all("""
            SELECT mc_p_tp, mc_p_sl, full_e_pnl, pnl_skewness,
                   signal_score, mc_var, volume_ratio,
                   funding_rate, oi_change_pct, btc_change_1h, rsi_14,
                   spread_pct, trend_sum, long_short_ratio, orderbook_imbalance,
                   volatility, pnl_pct, max_pnl_pct
            FROM position_snapshots
            WHERE pos_id = ? AND snapshot_at >= (
                SELECT opened_at FROM open_positions WHERE id = ?
            )
        ...
        sig = await pg_fetch_one("""
            SELECT s.re_recommendation, s.corrected_recommendation
        ...
        X = np.zeros((1, len(_ml_shadow_features)), dtype=np.float32)

        num_map = {
            "entry_vol": entry_row.get("volatility"),
            "entry_p_tp": entry_row.get("mc_p_tp"),
            "entry_p_sl": entry_row.get("mc_p_sl"),
            "entry_e_pnl": entry_row.get("full_e_pnl"),
            "entry_skewness": entry_row.get("pnl_skewness"),
            "entry_signal_score": entry_row.get("signal_score"),
            "entry_mc_var": entry_row.get("mc_var"),
            "entry_volume_ratio": entry_row.get("volume_ratio"),
            "entry_funding": entry_row.get("funding_rate"),
            "entry_oi_change": entry_row.get("oi_change_pct"),
            "entry_btc_1h": entry_row.get("btc_change_1h"),
            "entry_rsi": entry_row.get("rsi_14"),
            "entry_spread": entry_row.get("spread_pct"),
            "entry_trend": entry_row.get("trend_sum"),
            "entry_lsr": entry_row.get("long_short_ratio"),
            "entry_ob_imbal": entry_row.get("orderbook_imbalance"),
        }

        early_p_tp = [r["mc_p_tp"] for r in early_rows if r.get("mc_p_tp") is not None]
        early_p_sl = [r["mc_p_sl"] for r in early_rows if r.get("mc_p_sl") is not None]
        early_pnl = [r["pnl_pct"] for r in early_rows if r.get("pnl_pct") is not None]
        early_max = [r["max_pnl_pct"] for r in early_rows if r.get("max_pnl_pct") is not None]

        num_map["early_avg_p_tp"] = np.mean(early_p_tp) if early_p_tp else np.nan
        num_map["early_avg_p_sl"] = np.mean(early_p_sl) if early_p_sl else np.nan
        num_map["early_avg_pnl"] = np.mean(early_pnl) if early_pnl else np.nan
        num_map["early_max_pnl"] = max(early_max) if early_max else np.nan
        num_map["early_min_pnl"] = min(early_pnl) if early_pnl else np.nan
        num_map["early_pnl_std"] = float(np.std(early_pnl)) if len(early_pnl) > 1 else np.nan

        for j, feat in enumerate(_ml_shadow_numeric):
            val = num_map.get(feat)
            X[0, j] = float(val) if val is not None else np.nan

        base = len(_ml_shadow_numeric)
        X[0, base] = 1.0 if side == "long" else 0.0
        X[0, base + 1] = 1.0 if (sig and sig.get("re_recommendation") == "approve") else 0.0
        X[0, base + 2] = 1.0 if (sig and sig.get("corrected_recommendation") == "approve") else 0.0

        X_imp = _ml_shadow_imputer.transform(X)
        prob = float(_ml_shadow_model.predict_proba(X_imp)[0, 1])
        prediction = "PROFIT" if prob >= 0.5 else "LOSS"

        await pg_execute("""
            INSERT INTO ml_predictions (pos_id, symbol, side, snapshot_count,
                                        prob_profit, prediction, model_version, features_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (pos_id, symbol, side, snapshot_count,
              round(prob, 4), prediction, _ml_shadow_version,
              json.dumps({k: round(float(v), 4) if v is not None and not np.isnan(float(v)) else None
                          for k, v in num_map.items()})))

        _ml_shadow_logged.add(pos_id)

        confidence_tag = ""
        if prob >= config.ML_SHADOW_CONFIDENCE_THRESHOLD:
            confidence_tag = " [HIGH CONFIDENCE]"
        logger.info(
            f"[ML-SHADOW] {symbol} #{pos_id} ({side}): "
            f"prob_profit={prob:.3f} → {prediction}{confidence_tag} "
            f"(model={_ml_shadow_version}, snaps={snapshot_count})"
        )

    except Exception as e:
        logger.debug(f"[ML-SHADOW] Prediction failed for #{pos_id}: {e}")
...
NON_RETRIABLE_ERRORS = [
    ...
    "sl_price_invalid",       # v0.18.6: Bot SL pre-validation (bot v0.10.7)
    "move_tp failed",         # v0.18.6: TP move rejection by exchange
]
...
_bot_last_success_ts: float = 0.0           # v0.19.7: epoch of last successful /dumalka/positions response
...
async def _fetch_bot_active_symbols() -> set[str]:
    ...
    """
    global _bot_sync_failed, _bot_last_success_ts
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if r.status_code == 200:
                ...
                _bot_last_success_ts = time.time()
                return symbols
    except Exception as e:
...
async def send_command_to_bot(action: str, symbol: str, trace_id: str = None, **kwargs) -> dict:
    ...
    if not config.DUMALKA_ACTIVE_MODE:
        ...
        await _write_audit_log(trace_id, symbol, action + "_shadow",
                               kwargs.get("diagnostics"), pos_id=kwargs.get("_pos_id"))
        kwargs["_mode"] = "shadow"
    ...
    if sym_failures >= MAX_CONSECUTIVE_FAILURES:
        ...
        if elapsed >= BREAKER_COOLDOWN_SEC:
            ...
            await _write_audit_log(trace_id, symbol, action + "_breaker",
                                   kwargs.get("diagnostics"), pos_id=kwargs.get("_pos_id"))
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if r.status_code == 200 and result.get("ok"):
                    ...
                    await _write_audit_log(trace_id, symbol, action,
                                           kwargs.get("diagnostics"), pos_id=kwargs.get("_pos_id"))
                    ...
                    if _is_non_retriable_error(result):
                        # v0.18.6: Bot v0.10.7 sends position_alive=True when command
                        # is rejected but position still exists (e.g. SL price already
                        # past market). Do NOT increment phantom/desync counters.
                        _position_alive = result.get("position_alive", False)
                        if _position_alive:
                            logger.info(
                                f"🧠 Dumalka: {action} {symbol} rejected but position_alive=True: "
                                f"{result} (counters NOT incremented)"
                            )
                        else:
                                ...
                                if _cmd_fail_counts[fail_key] == CMD_FAIL_ALERT_THRESHOLD:
                                    ...
                                    try:
                                        ...
                                        await _write_audit_log(
                                            trace_id, symbol, "phantom_transition",
                                            pos_id=pid,
                                            details={
                                                "cmd_fail_count": _cmd_fail_counts.get(fail_key, 0),
                                                "last_action": action,
                                                "last_bot_response": str(result)[:300],
                                                "threshold": CMD_FAIL_ALERT_THRESHOLD,
                                            },
                                        )
                    ...
                    await _write_audit_log(trace_id, symbol, action + "_failed",
                                           kwargs.get("diagnostics"), bot_response=result,
                                           pos_id=kwargs.get("_pos_id"))
                return result
            ...
            await _write_audit_log(trace_id, symbol, action + "_error",
                                   kwargs.get("diagnostics"), bot_response={"connection_error": str(e)},
                                   pos_id=kwargs.get("_pos_id"))
            await _notify_telegram_action(action, symbol, trace_id, kwargs)
            ...
            await _write_audit_log(trace_id, symbol, action + "_error",
                                   kwargs.get("diagnostics"), bot_response={"exception": str(e)},
                                   pos_id=kwargs.get("_pos_id"))
            await _notify_telegram_action(action, symbol, trace_id, kwargs)
            return {"ok": False, "error": str(e)}
...
# v0.18.9 (2026-04-04): Zone 1 changed from sl_breakeven → hold.
#   Rationale: sl_breakeven at +1-2% profit causes shakeouts on micro-pullbacks,
#   killing potential rockets. Data: 5 positions stopped at BE averaged +0.24%
#   vs +3.40% if held 4h. RIVER #492 peaked at +8.76%, BE stopped at +0.09%.
#   Hard SL Cap (-3.5%) provides downside protection; Zone 1 BE is redundant.
DEFAULT_ZONE_POLICY = [
...
async def _write_audit_log(trace_id, symbol, action, diagnostics=None,
                           bot_response=None, pos_id=None, details=None):
    """Write to dumalka_audit_log DB table.

    v0.9.4 FIX-A: bot_response merged into mc_diagnostics JSON.
    v0.18.6: pos_id (direct FK to open_positions) and details (JSONB)
    for structured audit of closes, phantom transitions, state changes.
    """
    try:
        combined = {}
        ...
        details_json = json.dumps(details, cls=NpEncoder) if details else None
        market_state = json.dumps(_market_context) if _market_context else None
        await pg_execute(
            '''INSERT INTO dumalka_audit_log
               (timestamp, trace_id, symbol, action, mc_diagnostics,
                pos_id, details, market_state)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
            (datetime.now(timezone.utc).isoformat(), trace_id, symbol, action,
             diagnostics_json, pos_id, details_json, market_state)
        )
...
async def _notify_telegram_action(action, symbol, trace_id, kwargs):
    ...
    try:
        ...
        if action in ("full_close", "portfolio_override"):
            ...
            action_safe = _escape_md(action.upper())
            msg = (
                f"🚨 *ДУМАЛКА: {action_safe}*\n"
                f"Symbol: {symbol}\n"
            )
            if fraction:
...
async def _send_telegram_direct(msg: str) -> bool:
    """Send Dumalka Telegram alerts (digest, critical). Proxy first, then direct API."""
    return await send_telegram_message(msg, parse_mode="Markdown")
...
async def _flush_telegram_digest():
    ...
    for sym, actions in by_symbol.items():
        ...
        for act, cnt in action_counts.items():
            parts.append(f"{_escape_md(act)} x{cnt}")
        summary = ", ".join(parts)
    ...
    while True:
        try:
            ...
            _market_context.update({
                "regime": current_regime or "normal",
                "btc_1h": round(btc_change, 3),
            })
            ...
            for row in positions:
                ...
                if not _bot_sync_failed and symbol not in _bot_confirmed_symbols:
                    ...
                    if _age_sec > BOT_VERIFY_GRACE_SEC:
                        ...
                        if pos_id not in _observe_only_audited:
                            _observe_only_audited.add(pos_id)
                            await _write_audit_log(
                                pos.get("signal_hash"), symbol, "observe_only",
                                pos_id=pos_id,
                                details={"age_sec": _age_sec},
                            )
                ...
                if not _observe_only:
                    ...
                    if phantom_count >= PHANTOM_THRESHOLD:
                        # v0.18.5: Retry — bot may have restarted and not yet restored trades from DB.
                        # Wait 5s and re-check /dumalka/positions before committing to phantom close.
                        await asyncio.sleep(5)
                        _retry_syms = await _fetch_bot_active_symbols()
                        if symbol in _retry_syms:
                            _no_trade_count[pos_id] = 0
                            logger.info(
                                f"🔮 [PHANTOM RETRY] Pos #{pos_id} {symbol}: "
                                f"reappeared in bot after retry, counter reset"
                            )
                            await _write_audit_log(
                                pos.get("signal_hash"), symbol, "phantom_retry_ok",
                                pos_id=pos_id,
                                details={"old_phantom_count": phantom_count},
                            )
                            continue
                ...
                if current_pnl_pct < -config.MAX_LOSS_PCT:
                    if config.PHASE1_ACTIVE_MODE:
                        ...
                        await _close_position(pos_id, "hard_sl_cap", current_pnl_pct, current_price,
                                              max_pnl_pct, drawdown_from_peak_pct, tp_progress_pct,
                                              symbol=symbol, trace_id=signal_hash)
                        asyncio.create_task(insert_trade_outcome(
                            signal_hash=signal_hash,
                ...
                # v0.18.4: Idempotency guard — skip move_sl if SL already at BE or better.
                # v0.19.4 (2026-04-05): ATR-adaptive skip — when TP1 is within noise zone
                #   (tp1_dist < ATR(1h) × TP1_BE_ATR_SKIP_MULT), keep original SL.
                #   Validated: 7/7 cases since 27.03 — skip equal or better (+21.9% PnL).
                if tp1:
                    ...
                    if tp1_hit:
                        ...
                        if not _observe_only:
                            atr_1h_tp1 = current_vol / math.sqrt(8760)
                            tp1_dist = abs(tp1 - entry) / entry if entry > 0 else 0

                            if tp1_dist < atr_1h_tp1 * config.TP1_BE_ATR_SKIP_MULT:
                                logger.info(
                                    f"🎯 Pos #{pos_id} {symbol}: TP1 HIT but within "
                                    f"{config.TP1_BE_ATR_SKIP_MULT}×ATR noise zone "
                                    f"(tp1_dist={tp1_dist*100:.2f}% < "
                                    f"{atr_1h_tp1*100*config.TP1_BE_ATR_SKIP_MULT:.2f}%) "
                                    f"— keeping original SL, skipping BE move "
                                    f"(pnl={current_pnl_pct:.2f}%)"
                                )
                            else:
                                soft_be_offset = max(
                                    config.TP1_BE_MIN_PCT,
                                    min(config.TP1_BE_MAX_PCT, atr_1h_tp1 * config.TP1_BE_OFFSET_MULT),
                                )
                                be_sl = entry * (1 + soft_be_offset) if side == "long" else entry * (1 - soft_be_offset)

                                current_sl_val = pos.get("current_sl")
                                _sl_already_at_be = False
                                if current_sl_val is not None:
                                    _csv = float(current_sl_val)
                                    if side == "long" and _csv >= be_sl * 0.999:
                                        _sl_already_at_be = True
                                    elif side == "short" and _csv <= be_sl * 1.001:
                                        _sl_already_at_be = True

                                if not _sl_already_at_be:
                                    logger.info(
                                        f"🎯 Pos #{pos_id} {symbol}: TP1 HIT! → SL→BE "
                                        f"(pnl={current_pnl_pct:.2f}%, be_sl={be_sl:.6f}, "
                                        f"offset={soft_be_offset*100:.1f}%)"
                                    )
                                    result_sl = await _send_bot(
                                        ...
                                        diagnostics={"trigger": "tp1_hit", "pnl": current_pnl_pct,
                                                     ...
                                                     "atr_1h": atr_1h_tp1, "tp1_dist": tp1_dist},
                                        _pos_id=pos_id,
                                    )
                                    if result_sl.get("ok"):
                ...
                #
                # v0.18.4: "Proven Position" bypass — sticky latch.
                # NOM #548 post-mortem: position reached +14.14% (Zone 4, TP 100%) in 17 min
                # but grace blocked zone_full_exit → SL hit at -3.71%, NOM then went +35%.
                # Fix: once max_pnl >= 3% AND tp_progress >= 50%, mark proven (latched).
                # Proven positions get zone policy + trailing even during grace period.
                # Risk analysis (538 positions): only NOM #548 affected; 0 false positives.
                ...
                if _observe_only:
                    ...
                    if pos_id not in _proven_positions:
                        if max_pnl_pct >= TRAILING_ACTIVATION_PCT and tp_progress_pct >= 50.0:
                            _proven_positions.add(pos_id)
                            logger.info(
                                f"🏆 [GRACE PROVEN] Pos #{pos_id} {symbol}: marked as proven "
                                f"(max={max_pnl_pct:.1f}%, tp={tp_progress_pct:.0f}%) "
                                f"→ zone policy + trailing now active despite {hours_open:.2f}h age"
                            )
                            await _write_audit_log(
                                pos.get("signal_hash"), symbol, "grace_proven",
                                pos_id=pos_id,
                                details={"max_pnl": round(max_pnl_pct, 2),
                                         "tp_progress": round(tp_progress_pct, 1),
                                         "hours_open": round(hours_open, 2)},
                            )
                    if pos_id in _proven_positions:
                        pass  # zone policy, trailing, E_pnl all run normally
                    else:
                ...
                if (not _skip_active_gates
                    ...
                    if not momentum_alive:
                        ...
                        if config.APOLLO_BAIL_ENABLED:
                            ...
                            await _write_audit_log(
                                signal_hash, symbol, "apollo_shadow",
                                pos_id=pos_id,
                                details={"hours_open": round(hours_open, 2),
                                         "pnl": round(current_pnl_pct, 2),
                                         "full_e_pnl": round(full_e_pnl_pct, 2),
                                         "p_tp": round(p_tp, 4) if p_tp else None,
                                         "p_sl": round(p_sl, 4) if p_sl else None},
                            )
                        ...
                        await _write_audit_log(
                            pos.get("signal_hash"), symbol, "time_decay_exempt",
                            pos_id=pos_id,
                            details={"hours_open": round(hours_open, 2),
                                     "tp_progress": round(tp_progress_pct, 1),
                                     "pnl": round(current_pnl_pct, 2),
                ...
                if _e_pnl_would_close:
                    ...
                    result = await _send_bot(
                        ...
                        diagnostics={
                            "p_tp": p_tp, "p_sl": p_sl, "e_pnl": e_pnl_pct,
                            "full_e_pnl": full_e_pnl_pct, "skew": pnl_skewness,
                            "volatility": current_vol, "vol_ratio": volume_ratio,
                        },
                        _pos_id=pos_id,
                    )
                    if result.get("ok"):
                        ...
                        if zone_action == "full_close":
                            ...
                            result = await _send_bot(
                                ...
                                diagnostics={
                                    ...
                                    "zone": zone["name"], "base_thresh": zone["dd_thresh"],
                                    "af": adaptive_factor, "vol_mod": vol_modifier,
                                    "regime_dd": regime_dd_sensitivity, "heat": heat_modifier,
                                    "volatility": current_vol, "vol_ratio": volume_ratio,
                                },
                                _pos_id=pos_id,
                            )
                            ...
                            # v0.18.9: Clamp be_sl between entry and current_price so
                            # the SL is always on the valid side for Bybit.
                            ...
                            # v0.19.5: Skip if position is underwater — BE SL would be
                            # above current price (LONG) or below (SHORT), which Bybit
                            # rejects and the bot flags as sl_price_invalid.
                            _be_valid = (side == "long" and be_sl < current_price) or \
                                        (side == "short" and be_sl > current_price)
                            if not _be_valid:
                                logger.debug(
                                    f"🔕 Pos #{pos_id} {symbol}: sl_breakeven skipped — "
                                    f"be_sl={be_sl:.6f} wrong side of price={current_price:.6f} "
                                    f"for {side} (position underwater)"
                                )
                            else:
                                if side == "long":
                                    be_sl = min(be_sl, current_price * 0.999)
                                else:
                                    be_sl = max(be_sl, current_price * 1.001)
                                ...
                                if current_sl is not None and abs(float(current_sl) - be_sl) / entry < 0.0003:
                                    ...
                                    result = await _send_bot(
                                        ...
                                        zone=int(zone['name'].replace('Zone ', '')),
                                        diagnostics={
                                            "trigger": "sl_breakeven", "zone": zone['name'],
                ...
                asyncio.create_task(insert_position_snapshot(
                    ...
                    current_tp1=float(tp1) if tp1 else None,
                    current_tp3=float(tp3) if tp3 else None,
                ))
                ...
                # v0.19.6: ML Shadow prediction — once per position, after ~5 snapshots
                if (hours_open >= 0.04  # ~2.5 min = 5 cycles at 30s
                        and pos_id not in _ml_shadow_logged
                        and _ml_shadow_model is not None):
                    snap_n = await pg_fetch_val(
                        "SELECT COUNT(*) FROM position_snapshots WHERE pos_id = ?",
                        (pos_id,)
                    )
                    if snap_n and int(snap_n) >= 5:
                        asyncio.create_task(_ml_shadow_predict(
                            pos_id=pos_id, symbol=symbol, side=side,
                            snapshot_count=int(snap_n),
                        ))
...
async def _close_position(pos_id, reason, pnl, price, max_pnl, dd, tp_prog,
                          symbol=None, detail=None, trace_id=None):
    """Mark position as closed in DB + write structured audit entry.

    v0.18.6: Writes position_closed to dumalka_audit_log with full decision
    context (reason, PnL, max_pnl, drawdown, tp_progress).
    """
    _apollo_bail_attempted.pop(pos_id, None)
    _no_trade_count.pop(pos_id, None)
    _bot_absent_streak.pop(pos_id, None)
    _proven_positions.discard(pos_id)
    _observe_only_audited.discard(pos_id)
    ...
    # v0.18.6: Structured audit for every close — queryable post-mortem
    await _write_audit_log(
        trace_id, symbol or "UNKNOWN", "position_closed",
        pos_id=pos_id,
        details={
            "close_reason": reason,
            "detail": detail,
            ...
            "price": price,
            "max_pnl_pct": round(max_pnl, 4) if max_pnl is not None else None,
            "drawdown_pct": round(dd, 4) if dd is not None else None,
            "tp_progress_pct": round(tp_prog, 4) if tp_prog is not None else None,
        },
    )
    asyncio.create_task(update_snapshot_future_pnl(pos_id, pnl))
    asyncio.create_task(finalize_snapshot_future_pnl(pos_id, pnl, max_pnl_pct=max_pnl))
    if symbol:
...
async def admin_force_close_stale_position(pos_id: int, detail: str = "manual admin force close") -> dict:
    ...
    await _close_position(
        ...
        max_pnl, dd, tp, symbol=symbol, detail=detail, trace_id=signal_hash,
    )
    asyncio.create_task(insert_trade_outcome(
        signal_hash=signal_hash,
